...
func TestCmdKill_FlagsAfterPositional_RedisPath(t *testing.T) {
	...
	withArgs([]string{"eunox", "kill", "all", "--redis-addr", mr.Addr()}, func() { code = cmdKill(subArgsForTest()) })
	if code != 0 {
	...
	mr.FlushAll()
	withArgs([]string{"eunox", "kill", "--redis-addr", mr.Addr(), "all"}, func() { code = cmdKill(subArgsForTest()) })
	if code != 0 {
