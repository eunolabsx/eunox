...
func TestStampResultShape(t *testing.T) {
	...
	withRevision(t, revisionDeclaring, func() {
		...
		// Deliberately the WRONG answer for a response eunox filters: the clamp to `private`
		// is W5's, and a mock that volunteered `private` could not tell a working clamp from
		// an absent one.
