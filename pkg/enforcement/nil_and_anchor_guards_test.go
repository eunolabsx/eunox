...
// exported seams that answer with an error rather than an EnforceResponse.
//
// These are the seams a COMPOSING layer reaches: the PDP's claim-condition leg calls
// NonCommittingConditionVerdict before the deciding PDP runs, and its audit-mode antecedent
// path calls PeekSessionLabels/RecordSourceCall on a forwarded observe deny. All three handed
// req straight to code that dereferences it — the pure handlers read req.Context.SourceIP and
// req.Arguments, recordLabels reads matched.Directives and req.SessionID — so an embedder's bug
// crashed the process from inside the decision point instead of denying. Both callers already
// fail closed on the answers asserted here, so the guard needs no new branch in them.
func TestExportedStateSeams_NilArgumentsRefuseRatherThanPanic(t *testing.T) {
	...
	// Every pure handler that dereferences the request, so the guard is pinned at the seam
	// rather than at whichever handler happened to be dispatched.
	t.Run("NonCommittingConditionVerdict with a nil request", func(t *testing.T) {
		...
		conds := []capability.Condition{
			...
			capability.SequenceBlockCondition{AfterTools: []string{"read_secrets"}},
			capability.FlowLabelCondition{Allow: []string{"public"}},
			capability.AllowedOperationsCondition{Argument: "op", Operations: []string{"read"}},
		}
		for _, cond := range conds {
			t.Run(cond.ConditionType(), func(t *testing.T) {
				var ok bool
				assert.NotPanics(t, func() {
					_, ok = eng.NonCommittingConditionVerdict(ctx, cond, nil)
				...
				// The package-level entry point (a JWT-only or wiretap route, which holds no
				// engine) takes the same guard.
				assert.NotPanics(t, func() {
					_, ok = enforcement.NonCommittingConditionVerdict(ctx, cond, nil)
				})
				assert.False(t, ok)
			})
		}
	...
	t.Run("PeekSessionLabels with a nil request", func(t *testing.T) {
		t.Parallel()
		var err error
		assert.NotPanics(t, func() {
			_, err = eng.PeekSessionLabels(ctx, nil)
		})
		// The caller turns any error here into a hard ENFORCEMENT_ERROR deny, so reporting one
		// is all this seam owes; a nil error would read as "no labels carried" and fail open.
		require.Error(t, err)
	})

	t.Run("RecordSourceCall with a nil argument", func(t *testing.T) {
		t.Parallel()
		matched := &capability.Constraint{
			Target:     "tool:t",
			Actions:    []string{"call"},
			Directives: []capability.Directive{capability.LabelOutputDirective{Labels: []string{"confidential"}}},
		}
		...
		for _, tc := range []struct {
			...
			{"nil request", nil, matched},
			{"nil constraint", req, nil},
			{"both nil", nil, nil},
		} {
			t.Run(tc.name, func(t *testing.T) {
				...
				var labels []string
				assert.NotPanics(t, func() {
					labels, cerr = eng.RecordSourceCall(ctx, tc.req, tc.matched, scope, nil)
				})
				...
				assert.True(t, cerr.Flow, "Flow follows the scope the caller asked for, as the anchor backstop's does")
			})
		}

		// The bound on that guard: a nil constraint is refused only where the commit would
		// READ it. The PDP's no-match forwarded-state path passes nil deliberately — nothing
		// was selected, and with no source labels there is no synthetic constraint to build —
		// and refusing it turned an observe route's antecedent record into a hard deny, which
		// is the very fail-open-to-fail-shut inversion this seam must not cause.
		t.Run("antecedent-only commit accepts a nil constraint", func(t *testing.T) {
			labels, cerr := eng.RecordSourceCall(ctx, req, nil,
