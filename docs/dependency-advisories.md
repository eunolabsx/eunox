...
since moved to `go1.27.1`; that scan is CI's to run, and this register has not been
re-derived against it. The entries below are therefore a record of advisories that were
*reachable in the module graph* and what was concluded about each, not a list of things
the scan is currently printing.
