...
| `sampling/createMessage` *(upstream→host)* | **Enforced** (local subprocess upstream only). Refused before the policy decision, with no quota charged, if its own JSON-RPC id exceeds what the in-flight tracker retains (8 KiB) — see the row below — denied by default; requires explicit `system:sampling/createMessage` entry in manifest, and the kill switch applies. On the allow path the host's response is routed back to the upstream so the round-trip completes. A response the session gates refuse unblocks that upstream request only when the sender is PROVABLY the session's own owner (the audience pin can refuse an owner's own token); a reply from anyone else, or on a session with no bound identity, leaves the request for the owner's reply or for teardown, since answering it would let a second identity that learned a session id abort the owner's pending reply | Fail-closed: absent = deny. See the transport caveats in *Known gaps* below. On **both** hosts each server-initiated request is handled on its own goroutine rather than on the upstream reader, so a request waiting for the decision turn cannot stall response delivery: independent server-initiated requests consequently have **no ordering guarantee** among themselves, and at most 32 may be in flight at once — per proxy on stdio (one upstream), per SESSION on HTTP (one upstream subprocess each) — beyond which the upstream is answered with a retryable server-busy error (`-32000`) and the refusal is recorded `RESOURCE_EXHAUSTED` — and if that answer itself cannot be delivered, an `ENFORCEMENT_ERROR` deny tagged `transport: {http,stdio}-server-refusal-undeliverable` beside it |
| Upstream-initiated non-sampling requests (e.g. `roots/list`) | **Forwarded** for local subprocess upstreams — not policy-enforced (no allow/deny decision), but kill-switch-checked, `--require-audit=strict`-gated, and audited (an allow record on delivery, an `ENFORCEMENT_ERROR` deny if no client received it, and an `ENFORCEMENT_ERROR` deny tagged `transport: {http,stdio}-server-request-displaced` if the proxy's 1024-entry in-flight tracker had to displace it — at its capacity limit, or because the upstream reused a request id — in which case its initiator is answered `-32603` rather than left blocked, since nothing could route a reply to it afterwards; and an `ENFORCEMENT_ERROR` deny tagged `transport: {http,stdio}-server-request-id-unroutable` if the request's own JSON-RPC id is larger than the tracker retains (8 KiB), in which case it is answered the same way and **not** forwarded to the host at all, since no reply to it could be routed back); the host's response is routed back to the upstream — and if that reply cannot be relayed because the session has no upstream sink (remote-upstream mode), the destroyed reply is recorded as an `ENFORCEMENT_ERROR` deny tagged `transport: {http,stdio}-server-reply-undeliverable`. Any answer EUNOX ITSELF produces on this leg that never reaches the initiator — the upstream sink failed or was absent while eunox was declining the request, answering a displaced or unroutable-id request, refusing a host reply on protocol-revision grounds, reporting an SSE delivery that failed after buffering, or reporting that no client stream could take the request — is recorded the same way, tagged `transport: {http,stdio}-server-refusal-undeliverable`, beside the record naming the reason for the answer: that first record describes the refusal, not the delivery, so without the second an operator cannot tell a peer that was told from one still blocked | Remote HTTP upstreams have no background reader and do not consume server-initiated messages. These share the concurrency bound and the ordering caveat noted for `sampling/createMessage` above, on both hosts |
