...
- Two case keys that fold together (`DROP` and `drop`) are **rejected at load**: one value
  cannot resolve to two effects, and leaving the tie to the matcher's own collision order
  would make the verdict nondeterministic. The fold is the matcher's — Unicode simple case
  folding after trimming, not a lower-casing — so `select` and `ſelect` (U+017F LATIN SMALL
  LETTER LONG S) collide too: the argument `SELECT` matches both at runtime.
