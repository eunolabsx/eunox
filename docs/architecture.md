...
| `pkg/circuitbreaker` | Generic circuit breaker (`Do` around a fallible call). Currently guards JWKS endpoint fetches; its `Stats` are read back through `capability.(*JWKSCache).KeyFetchHealth` — which joins them with the cache's own staleness, since a tripped breaker over a warm cache rejects nothing — and reported on the HTTP transport's `/healthz` and `/metrics`. `State.Impeded` lives here rather than in the consumer, so what the states mean is defined beside the state machine. |
...
| Distributed state | Implement the `callcounter` / `killswitch` store interfaces (Redis backends are the reference). For `killswitch`, implement the full `Manager` — including `HealthStatus() error` — and honour its CONFIRMABILITY rule: a backend that cannot confirm its kill set must report that cause from every reader, since `(false, nil)` is indistinguishable from a confirmed all-clear. An in-process backend is always confirmable and returns nil; do not stub `HealthStatus` to nil on one that mirrors remote state. `pkg/killswitch`'s conformance table is the executable form of the contract. For `callcounter`, implement the single mandatory `capability.CallCounter` contract — `Peek` backs `sequenceBlock`, `AdmitAll` backs every quota bound (`maxCalls` and the cumulative `blastRadius`, over entry-counting and weight-summing buckets respectively) — and pin conformance with one `var _ capability.CallCounter = (*MyStore)(nil)` assertion so an omitted method is a build error rather than an opaque runtime deny. |
