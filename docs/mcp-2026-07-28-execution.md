...
SERVED over HTTP with no `initialize` anywhere in the exchange. The worker is keyed on the caller's
stable identity resolved under the route's own state anchor (`enforcement.ResolveStateAnchor`,
route-namespaced) — the same subject stateful policy keys on, so the worker map and the accumulated
state cannot disagree about a request, and a rotated credential lands on the same worker because
`jti` stays revocation-only. On the session arm the identity IS the anchor; on a task-anchored route
...
All three exit criteria are ticked, which by this document's rule makes the workstream
landed. Scope it does NOT cover is listed here rather than left implied — each item is
blocked on another workstream rather than unstarted, and none of them is an exit criterion:

- The **Docker half of the demo walkthrough**. The demo mocks and the stateless
  `make -C demo stateless-…` targets have since landed separately, so this is no longer "the
  walkthrough is old-revision only" — it is the compose wiring alone, and it is blocked on W2
  rather than unstarted: an HTTP session is opened by `initialize`, so a 2026-07-28 pin is
  refused over the HTTP host transport rather than accepted into a config where every
  forwarded request would then be refused. The HTTP mock already has its `--protocol-version`
  mode and its tests, so the fixture is waiting on W2 rather than the other way round.
