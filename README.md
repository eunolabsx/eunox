...
| `--upstream-protocol-version` | In `--audit` wiretap mode, pins the MCP protocol revision spoken to an `--upstream-url`: `auto` probes it from the upstream's own handshake, or name a revision. The config-file equivalent is an upstream's `protocolVersion` key. | `auto` |
