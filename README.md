...
<p align="center">
  <a href="https://go.dev/"><img alt="go version" src="https://img.shields.io/badge/go-%E2%89%A51.27.1-00ADD8"></a>
...
| **`descriptionHash` pin not satisfied** | `ERROR` | A live tool's description — **or any of its parameter descriptions** — doesn't match the `descriptionHash` pinned for it; the model-facing instruction text may have been altered to steer the model toward unintended calls (a poisoning attempt). **Always fatal**: aborts session establishment unconditionally, with or without `--strict-drift`. |
| **Description-pinned tool absent from the live list** | `ERROR` | A `tool:` entry carrying a `descriptionHash` matches no live tool, so its pin could not be verified this session — an upstream hiding a poisoned tool looks exactly like a removed one. **Always fatal**, like the mismatch above. |
...
| ------- | ------- | ------- | ----------- |
| **Runaway sessions** — each session owns an upstream subprocess/connection; an unbounded client could spawn many. | Cap concurrent sessions. The slot is taken before the upstream is spawned and held across the whole handshake, so a session past the cap gets `503` with nothing spawned — concurrent `initialize`s cannot all clear the check and start an upstream each. | **512** backstop (set `0` for unlimited, via either knob) | `listen.maxSessions` / `--max-sessions` |
...
| **A unique-key flood pinning heap** — the in-memory `maxCalls`/`sequenceBlock` counter holds one key per live `(session, tool)` pair. | Cap distinct counter keys; a call under a new key past the limit fails closed. Refused with `--redis-addr` (Redis holds counter state off-heap with TTLs, so the ceiling would configure nothing). | **1,000,000** (`0` disables) | `--max-call-counter-keys` |
...
| `--upstream-protocol-version` | In `--audit` wiretap mode, pins the MCP protocol revision spoken to the upstream, which selects how its leg is opened: `auto` opens with the `initialize` handshake, or name a revision. Works on either upstream transport. The config-file equivalent is an upstream's `protocolVersion` key. | `auto` |
...
| `eunox kill [--port N] [--host H] <session-id\|all>`                | Revoke one or all active sessions. POSTs to an HTTP proxy's loopback control endpoint by default — authenticated with the control token the proxy writes to `~/.eunox/control.token` (0600) at startup, which `kill` reads automatically (override with `--control-token` / `--control-token-path` / `EUNOX_CONTROL_TOKEN`). That endpoint accepts loopback sources only, so `--host` must be a loopback spelling — anything else is rejected before the token is read, rather than presented to whatever is listening there. With `--redis-addr` it writes the kill to shared Redis state instead — the way to revoke a stdio proxy started with `--redis-addr`, and the fan-out switch across every instance on that Redis. A plain in-memory stdio proxy has no out-of-band kill: stop the process. |
| `eunox kill --revive --redis-addr <addr> <session-id\|all>`         | Lift a revocation: remove one session's kill tombstone, or (with `all`) deactivate the global kill switch, leaving per-session and per-agent kills in place. Redis-only — the loopback control endpoint is a one-way emergency stop, and an in-memory kill switch is cleared by restarting the proxy. |
| `eunox kill [--revive] --redis-addr <addr> --agent <agent-id>`      | Revoke (or lift the revocation of) a JWT agent identity across every session it holds — the right granularity when one compromised `agent_id` spans many sessions, instead of killing each session id or reaching for the global switch. Agent kills never expire, so `--revive` is the only way to lift one. Redis-only: there is no agent dimension on the loopback control endpoint. |
...
Prerequisites (Go 1.27.1+), repository layout, and the CI matrix
