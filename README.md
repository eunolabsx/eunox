...
| `eunox contracts` | Verify a local effect-contract corpus (every declared digest recomputed from its own content) and print the `effect.ref` pin an author copies into a manifest. Offline — the registry is never fetched. |
