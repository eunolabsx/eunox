...
// TestTruncateUTF8 exercises the exported normalize-then-rune-safe-cut helper
// boundFieldTo delegates to and internal/transport's sanitizeClaimedID also calls
// directly, so both copies of the "make an attacker string safe to log" logic share
// one set of tests.
func TestTruncateUTF8(t *testing.T) {
	...
	// Non-positive limit truncates to "", with no marker (unlike boundFieldTo).
	...
	// Cuts to exactly the byte limit, never over, and never carries boundFieldTo's
	// visible marker.
	s := strings.Repeat("x", 1000)
	...
	if strings.Contains(got, "truncated") {
		t.Errorf("TruncateUTF8 must not add boundFieldTo's marker, got %q", got)
	}
