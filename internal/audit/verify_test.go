...
// TestOpenAuditSink_NonCanonicalTailIsNotLabelledHMACMismatch pins the same forensic
// taxonomy one step earlier than the strict-decode case above. The canonical-on-disk-form
// check runs BEFORE any MAC is computed, so a tail whose bytes were rewritten after
// signing — here by inserting whitespace inside the object, which decodes to the very same
// fields — is refused with no HMAC comparison ever having run, yet the resume recorded it
// as tail_hmac_mismatch. It gets its own kind rather than the strict decoder's: that one
// says the line could not be read, this one says it read fine and was rewritten.
func TestOpenAuditSink_NonCanonicalTailIsNotLabelledHMACMismatch(t *testing.T) {
	dir := t.TempDir()
	logPath, keyPath := writeChainLog(t, dir, "read_file", "write_file")

	// Whitespace inside the object: the lenient AND strict decoders both accept it and see
	// the signed fields unchanged, so the line reaches the canonical check — which is the
	// only thing that rejects it.
	lines := logLines(t, logPath)
	tampered := bytes.Replace(lines[1], []byte(`{`), []byte(`{ `), 1)
	if bytes.Equal(tampered, lines[1]) {
		t.Fatal("injection did not change the tail line")
	}
	var claimed auditRecord
	if err := json.Unmarshal(tampered, &claimed); err != nil {
		t.Fatalf("the rewritten tail must still decode: %v", err)
	}
	rewriteLines(t, logPath, [][]byte{lines[0], tampered})
	...
	details := recordDetails(t, marker)
	if got := details["kind"]; got != "tail_non_canonical" {
		t.Errorf("marker kind = %v, want tail_non_canonical — no HMAC comparison ran, so the marker must not claim one failed", got)
	}
	// The claimed_ fields locate the suspect record without vouching for it.
	if got, ok := details["claimed_tail_seq"].(float64); !ok || uint64(got) != claimed.Seq {
		t.Errorf("marker Details[claimed_tail_seq] = %v, want %d", details["claimed_tail_seq"], claimed.Seq)
	}
	if got := details["claimed_tail_hmac"]; got != claimed.HMAC {
		t.Errorf("marker Details[claimed_tail_hmac] = %v, want %q", got, claimed.HMAC)
	}
	// Fail-closed behavior is unchanged: the untrusted tail never seeds the chain.
	if marker.PrevHMAC != auditGenesisPrev {
		t.Errorf("marker PrevHMAC = %q, want the genesis sentinel %q", marker.PrevHMAC, auditGenesisPrev)
	}
	if marker.Seq != 1 {
		t.Errorf("marker Seq = %d, want 1 (genesis restart; the rewritten tail must not seed the chain)", marker.Seq)
	}
	if marker.DenialCode != "AUDIT_INTEGRITY_FAILURE" {
		t.Errorf("marker DenialCode = %q, want AUDIT_INTEGRITY_FAILURE", marker.DenialCode)
	}
...
// share. The read-only caller (interpretAuditTail) fails closed on it with dedicated tests; the
// ACTIVE log's two reads accepted it — buf = buf[:n] — so a file externally truncated between
// the Stat that sizes the read and the ReadAt that performs it (a rotation daemon racing a
// restart; the audit lock excludes other eunox writers, not other tools) was processed as
// authoritative.
func TestTailReadShrank_OneInterpretationForThreeReads(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// reader refuses on its own, ahead of this check.
...
// TestReadTailWindow_RefusesAShrunkRead covers the guard at the PROBE's read — the site whose
// own Stat and ReadAt sit inside one function, which is why the read takes size as a parameter:
// a stale size is exactly what an external truncation produces, and without the seam the guard
// could be deleted, misplaced below the slice, or given transposed arguments with nothing
// failing. Deleting it does fail this test.
func TestReadTailWindow_RefusesAShrunkRead(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	logPath := filepath.Join(dir, "audit.jsonl")
	content := `{"seq":1,"decision":"allow"}` + "\n"
	if err := os.WriteFile(logPath, []byte(content), 0o600); err != nil {
		t.Fatalf("write log: %v", err)
	}
	f, err := os.Open(logPath) //nolint:gosec // G304: test-controlled path
	if err != nil {
		t.Fatalf("open: %v", err)
	}
	defer func() { _ = f.Close() }()

	// A whole read answers with the window and no error.
	got, err := readTailWindow(f, 0, int64(len(content)))
	if err != nil || string(got) != content {
		t.Fatalf("whole read = (%q, %v), want (the window, nil)", got, err)
	}
	// A size larger than the file is what a Stat taken before an external truncation reports.
	// It must refuse rather than hand back the short read.
	got, err = readTailWindow(f, 0, int64(len(content))+64)
	if got != nil {
		t.Errorf("a refused read must return no window, got %q", got)
	}
	// errAuditTailProbe is the disposition recoverPartialTail already fails Open closed on;
	// errAuditTailShrunk is the cause its one retrying caller reads.
	if !errors.Is(err, errAuditTailProbe) {
		t.Fatalf("error = %v, want it to wrap errAuditTailProbe", err)
	}
	if !errors.Is(err, errAuditTailShrunk) {
		t.Fatalf("error = %v, want it to wrap errAuditTailShrunk, which is what distinguishes a stale size from an I/O fault no retry would fix", err)
	}
}

// TestRefuseTruncateGrowth covers the ReadAt->Truncate half of the same race. newSize is derived
// from the Stat taken before the window read, and ftruncate to a size ABOVE the file's current
// one EXTENDS it with a NUL hole — so an unguarded truncation would grow a shrunk log back to
// newSize NUL bytes, land the next append past them, and resume the chain from a record no
// longer on disk. That is strictly worse than the misread the read's guard prevents: the file is
// corrupted, not just misinterpreted.
func TestRefuseTruncateGrowth(t *testing.T) {
	t.Parallel()
	dir := t.TempDir()
	logPath := filepath.Join(dir, "audit.jsonl")
	content := `{"seq":1,"decision":"allow"}` + "\n"
	...
	// A shortening truncation is the ordinary case and passes.
	if err := refuseTruncateGrowth(f, int64(len(content))-1); err != nil {
		t.Fatalf("a shortening truncation must be allowed, got %v", err)
	}
	// The file shrinks under the handle, as an external rotator does.
	if err := os.Truncate(logPath, 0); err != nil {
		t.Fatalf("truncate: %v", err)
	}
	err = refuseTruncateGrowth(f, int64(len(content)))
	if !errors.Is(err, errAuditTailProbe) || !errors.Is(err, errAuditTailShrunk) {
		t.Fatalf("error = %v, want it to wrap both errAuditTailProbe and errAuditTailShrunk", err)
	}
	// And the file is untouched: a refusal must not be the thing that grows it.
	fi, statErr := os.Stat(logPath)
	if statErr != nil || fi.Size() != 0 {
		t.Fatalf("size = %v (err %v), want 0 — the refusal must leave the file alone", fi.Size(), statErr)
	}
}
...
// back short with io.EOF, which this path used to slice and interpret.
//
// Refused rather than re-probed here, unlike the initial probe: this caller has already
// truncated, and its byte count is the operator's record of that.
