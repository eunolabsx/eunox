...
func TestLoadUpstreamPDP_ExpectVersionWithoutPolicy(t *testing.T) {
	...
	_, _, _, _, err := LoadUpstreamPDP(u, config.HostTransportStdio, "", callcounter.NewInMemory(), nil, killswitch.NewInMemory(), false)
	if err == nil {
...
func TestLoadUpstreamPDP_SamplingOptInOnHTTPUpstreamRejected(t *testing.T) {
	...
	_, _, _, _, err := LoadUpstreamPDP(u, config.HostTransportHTTP, "", callcounter.NewInMemory(), nil, killswitch.NewInMemory(), false)
	if err == nil {
