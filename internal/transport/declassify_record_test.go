...
func TestEnforcedForwardCore_StandingGrantStampsNoSpentID(t *testing.T) {
	...
	got := rec.records[0]
	assert.Equal(t, []string{capability.FlowLabelPII}, got.labelsCleared)
	assert.Nil(t, got.details[audit.DeclassifySpentApprovalKey])
	assert.Nil(t, got.details, "a clean standing-grant clear allocates no details at all")
}
...
func TestDispatchParams_WireTheCommitterFromTheDecidingPDP(t *testing.T) {
	...
	assert.Equal(t, d.pdp, d.committer, "and it must be the PDP that decided")
	...
	assert.Equal(t, hd.pdp, hd.committer, "and it must be the PDP that decided")
}
