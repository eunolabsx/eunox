...
// redactFields directive: mask JSON paths in a tools/call result.
//
// Applied POST-allow only, by the transports' forward path, to a result the PDP has
// already permitted. Split out of pdp.go so the decision logic there is not buried
// under the DLP machinery; behavior is unchanged and the entry point is
// ApplyRedactObligs.
