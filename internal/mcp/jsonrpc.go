...
// every later Write fails fast rather than interleaving bytes into a corrupt stream. On the
// poison transition the writer's onPoison hook fires (the transports wire it to tear the
// session down), so the desynced stream is reaped rather than reused — the recovery the
// deadline exists to enable.
var ErrUpstreamWriteTimeout = errors.New("mcp: upstream write deadline exceeded; stream framing desynced")

// ErrFrameDesync marks a write that flushed only PART of its frame for a reason other than
// a deadline — EPIPE, ENOSPC, or a signal interrupting a write larger than the pipe's
// atomic-write size. The framing consequence is identical to a timeout (the next frame
// would be appended onto half of the previous one), so it poisons the writer the same way,
// but the CAUSE is not a timeout and must not be reported as one: classifying an upstream
// that died mid-write as UPSTREAM_TIMEOUT puts a fabricated duration on the audit tape and
// tells the operator to raise --upstream-timeout. A distinct sentinel lets the
// classification layer tell them apart; the underlying error is wrapped alongside it.
...
type MsgWriter struct {
	...
	// regardless of which write path (request, notification, server-reply) hit it. A writer
	// with no hook has no owner to react, so every later write fails silently at call sites
	// that discard the error — set one on any writer whose stream matters.
	onPoison func()
	// poisonErr latches the reason the writer was poisoned (nil = healthy): the stream
	// framing is desynced, so all subsequent writes fail fast with this same error. Storing
	// the reason rather than a bool keeps a later write reporting the CAUSE that broke the
	// stream (deadline vs partial frame) instead of collapsing both onto one sentinel.
	poisonErr error
}
...
func (mw *MsgWriter) Write(msg RPCMsg) error {
	...
	// interrupted by EPIPE, ENOSPC, or a signal leaves n < len(data) just the same, and the
	// framing is equally desynced. Keyed on the byte count rather than on err, because a
	// short write is a desync whether or not the writer honored io.Writer's contract of
