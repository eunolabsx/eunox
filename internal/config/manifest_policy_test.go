...
// TestAccumulatesSharedState_LoadedManifestPointerCondition: a
...
// advisory never fired. The predicate must accept both the pointer and value forms —
// which it now gets for free, since the class is read off the discriminator the interface
// reports rather than off a Go type switch.
func TestAccumulatesSharedState_LoadedManifestPointerCondition(t *testing.T) {
	yaml := `
name: "quota-policy"
version: "1.0.0"
capabilities:
...
`
	...
	if !m.AccumulatesSharedState() {
		t.Error("AccumulatesSharedState() = false for a loaded manifest with a maxCalls condition; want true")
	}
}

// TestAccumulatesSharedState_ValueCondition covers the value-typed form (the JWT path builds
// value-typed conditions), so both shapes a condition can take are exercised.
func TestAccumulatesSharedState_ValueCondition(t *testing.T) {
	m := &LocalManifest{
	...
	if !m.AccumulatesSharedState() {
		t.Error("AccumulatesSharedState() = false for a value-typed maxCalls condition; want true")
	}
}

// TestAccumulatesSharedState_None confirms a manifest whose entries accumulate nothing
// reports false, so the advisory stays silent for a policy it does not apply to.
func TestAccumulatesSharedState_None(t *testing.T) {
	yaml := `
...
`
	...
	if m.AccumulatesSharedState() {
		t.Error("AccumulatesSharedState() = true for a manifest with no accumulating token; want false")
	}
	var nilManifest *LocalManifest
	if nilManifest.AccumulatesSharedState() {
		t.Error("AccumulatesSharedState() = true for a nil manifest; want false")
	}
	if nilManifest.NeedsDecisionTurn() {
		t.Error("NeedsDecisionTurn() = true for a nil manifest; want false")
	}
